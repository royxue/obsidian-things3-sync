...
* **Permanent Links**: Optionally link todos via the [Advanced URI](https://github.com/Vinzent03/obsidian-advanced-uri) plugin using a uid stored in the note's frontmatter (field configurable via *UID frontmatter field*, default `id`), so backlinks survive renames, moves, and duplicate note names (enable *Advanced URI mode* in the settings).
